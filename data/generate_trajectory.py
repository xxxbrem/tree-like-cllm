...
def main(filename, model, tokenizer, max_new_tokens, max_new_seq_len, use_aug, use_labels, data_size):
    ...
    new_file_name = "cleaned_" + f"{filename.split('/')[-1].split('.')[0].lower()}_{args.model.split('/')[-1]}_jacobi_max_new_tokens{max_new_tokens}_aug{use_aug}_labels_{use_labels}_max_seq_len_{max_new_seq_len}.json"
